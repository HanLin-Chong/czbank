...
(function ($) {
	...
	jQuery.fn.raytable = function (options) {
		...
		// add a column to hold row numbers
		if (base.rowNumbers) {
			options.columns.unshift({ field: "rowNum", title: "" ,width: 20});//here 修改序号
		}
	...
	function renderTable() {
		...
		//改
		base.parentElem.append('<div id="raytable-footer" style="padding:5px;"><span id="raytable-footer-summary" style="float:right;">0条记录</span></div>');
	}
	...
	function renderFooter(params) {
		...
		// update the summary
		base.parentElem.find('#raytable-footer-summary').text('第' + params.start + ' - ' + params.end + ' 条记录，共 ' + params.total + ' 条');
		...
		//改
		var summary = '<span id="raytable-footer-summary" style="float:right;">第' + params.start + ' - ' + params.end + ' 条记录，共 ' + params.total + ' 条</span>';
